# -*- coding: utf-8 -*-
"""SQLite persistence for TikTok scheduled uploads (separate from YouTube DB).

Publishing policy: max 3 per calendar day at hours 16, 18, 21 (America/Sao_Paulo).
See docs/tiktok/SCHEDULING_POLICY.md.
"""
...
from .config import resolve_schedule_db_path

DEFAULT_DB_PATH = resolve_schedule_db_path()
...
class ScheduleDB:
    ...
    def _init_schema(self) -> None:
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_tiktok_scheduled_slot
                    ON scheduled_uploads(slot_date, slot_hour, status);

                CREATE INDEX IF NOT EXISTS idx_tiktok_scheduled_status
        ...
        with self._connect() as conn:
            cur = conn.execute(
                ...
                INSERT INTO scheduled_uploads (
                    file_path, caption, scheduled_at_utc, slot_date, slot_hour,
                    status, created_at
                ) VALUES (?, ?, ?, ?, ?, 'pending', ?)
                """,
                (
    ...
    def mark_scheduled(self, row_id: int, post_id: str) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                UPDATE scheduled_uploads
                SET status = 'scheduled', post_id = ?, last_error = NULL
                WHERE id = ?
                """,
                (post_id, row_id),
            )

    def mark_uploaded(self, row_id: int, post_id: str) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                UPDATE scheduled_uploads
                SET status = 'uploaded', post_id = ?, last_error = NULL
                WHERE id = ?
                """,
                (post_id, row_id),
            )

    def mark_file_uploaded(self, file_path: Path, post_id: str, *, immediate: bool = True) -> bool:
        """Mark a clip as uploaded by file path (e.g. already posted outside schedule UI)."""
        row = self.get_by_file(file_path)
        if row is None:
            return False
        if immediate:
            self.mark_uploaded(row.id, post_id)
        else:
            self.mark_scheduled(row.id, post_id)
        return True
    ...
    def update_caption(self, row_id: int, caption: str) -> None:
        with self._connect() as conn:
            conn.execute(
                "UPDATE scheduled_uploads SET caption = ? WHERE id = ?",
                (caption[:4000], row_id),
            )
    ...
    def count_active_for_date(self, slot_date: str) -> int:
        """Rows occupying a calendar day (pending/uploading/scheduled/uploaded)."""
        with self._connect() as conn:
            row = conn.execute(
                """
